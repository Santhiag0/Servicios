...
const Invoice = ({ addedProducts, onRemoveProduct }) => {
  ...
  const [ivas, setIvas] = useState(0);

  const subtotal = addedProducts.reduce(
    (acc, product) => acc + product.price * product.quantity,
    0
  );
  const iva = subtotal * ivas;
  ...
  const fetchIVA = async () => {
    try {
      ...
      const response = await fetch(
        "https://facturacion-servicios.onrender.com/api/productos/5",
        {
          method: "GET",
          headers: {
      ...
      if (response.ok) {
        ...
        const iva = jsonResponse.iva.percent;
        setIvas(jsonResponse.iva.value);
        setIvaPercentage(iva);
      } else {
        throw new Error("Error getting IVA product");
      }
    } catch (error) {
      console.error(error);
      throw error;
  ...
  return (
    <div className="w-2/5 bg-white p-8 rounded-lg shadow-md">
      ...
      <div className="mb-4">
        <label htmlFor="customerType" className="block text-gray-700 mb-2">
          Tipo de Cliente
        </label>
        <select
          id="customerType"
        ...
        >
          <option value="consumer" className="text-gray-700">
            Consumidor Final
          </option>
          <option value="data" className="text-gray-700">
            Datos
          </option>
        </select>
      </div>
      {customerType === "data" && (
        <div className="mb-4">
          <label htmlFor="dni" className="block text-gray-700 mb-2">
            Cédula
          </label>
          <div className="flex">
            <input
      ...
      {customerType === "consumer" && (
        <div className="mt-4">
          ...
          <p className="text-gray-700">
            <strong>Dirección:</strong> Dirección Genérica
          </p>
          <p className="text-gray-700">
            <strong>Teléfono:</strong> 9999999999
          </p>
          <p className="text-gray-700">
            <strong>Email:</strong> consumidor@final.com
          </p>
        </div>
      )}
      <div className="mb-4">
        ...
        {addedProducts.map((product) => (
          ...
          >
            ...
            <span className="text-gray-800 w-1/5 text-center">
              {product.quantity}
            </span>
